...
<form class="ajaxform_instant_reload" method="post" action="{{ route('admin.blog.store') }}" enctype="multipart/form-data">
	@csrf
			...
			<div class="card">
				<div class="card-body">
					...
					<div class="from-group row mt-2">
						...
						<div class="col-lg-12">
							<textarea name="short_description"  class="form-control h-100" maxlength="500">{{ old('short_description')??'' }}</textarea>
						</div>
					</div>
					<div class="from-group row mt-3">
						<label  class="col-lg-12">{{ __('Main Description') }}</label>
						<div class="col-lg-12">
							<textarea name="main_description" rows="40"  class="summernote form-control">{{ old('main_description')??'' }}</textarea>
						</div>
					</div>
					<div class="from-group row  mt-2">
						...
						<div class="col-lg-12">
							<select name="categories[]" class="form-control select2" multiple="">
								...
								<option value="{{ $key }}" @selected(in_array($key,old('categories',[])))> {{ __($category) }} </option>
								@endforeach
							</select>
						</div>
					</div>
					<div class="from-group row  mt-2">
						...
						<div class="col-lg-12">
							<select name="categories[]" class="form-control select2" multiple="">
								...
								<option value="{{ $key }}"  @selected(in_array($key,old('tags',[])))> {{ __($tag) }} </option>
								@endforeach
							</select>
						</div>
					</div>
					<div class="from-group row  mt-2">
						...
						<div class="col-lg-12">
							<select name="language" class="form-control" >
								...
								<option value="{{ $languagesKey }}" @selected(old('language')==$languagesKey)> {{ $language }} </option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="from-group row mt-2">
						...
						<div class="col-lg-12">
							<textarea name="meta_description"  class="form-control h-100">{{ old('meta_description')??'' }}</textarea>
						</div>
					</div>
					<div class="from-group row mt-2">
					...
					<div class="from-group row  mt-3">
						<div class="col-lg-12 d-flex">
							...
							<label class="custom-toggle custom-toggle-primary">
								<input type="checkbox"  name="featured" value="1" id="plain-text-with-featured"  data-target=".plain-title-with-featured" class="save-template" @checked(old('featured'))>
						...
						<div class="col-lg-12 d-flex">
							<label class="custom-toggle custom-toggle-primary">
								<input type="checkbox"  name="status" value="1" id="plain-text-with-button"  data-target=".plain-title-with-button" class="save-template" @checked(old('status'))>
								<span class="custom-toggle-slider rounded-circle" data-label-off="No" data-label-on="Yes"></span>
							</label>

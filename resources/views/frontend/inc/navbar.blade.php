...
<nav class="navbar navbar-expand-lg  ">
    <div class="container-fluid  d-flex   justify-content-between align-items center">
        <a class="navbar-brand " href="#"> <img src="{{ asset('frontend') }}/assets/images/logo.svg"
                alt="av-sender"></a>
        ...
        <div class="collapse navbar-collapse w-100" id="navbarSupportedContent">
            ...
            <ul
                class="navbar-nav  justify-content-evenly m-auto  {{ app()->getLocale() == 'ar' ? 'flex-lg-row-reverse ' : '' }}">
                @if (app()->getLocale() == 'ar')
                    <li class="nav-item dropdown lang-option {{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }} ">
                        <a class="nav-link dropdown-toggle text-capitalize  " href="#" id="navbarDropdown"
                            ...
                            <img src="{{ asset('frontend') }}/assets/images/{{ app()->getLocale() == 'ar' ? 'arabic' : 'english' }}.png"
                                alt="language" class="rounded-circle lang-icon">
                            <span>{{ app()->getLocale() == 'ar' ? 'العربية' : 'English' }}</span>
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            @foreach (get_option('languages', true) as $code => $language)
                                <li class="mb-1 p-1">
                                    <a class="dropdown-item d-flex align-items-center  lang"
                                        href="{{ route('switchLang', $code) }}">
                                        @if ($language === 'Arabic')
                                            <img src="{{ asset('frontend') }}/assets/images/arabic.png"
                                                alt="Saudi Arabia Flag" class="me-3">
                                        @else
                                            <img src="{{ asset('frontend') }}/assets/images/english.png"
                                                alt="American Flag" class="me-3">
                                        @endif
